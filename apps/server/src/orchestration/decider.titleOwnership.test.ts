...
it.layer(NodeServices.layer)("recurring title ownership", (it) => {
  ...
  it.effect("same-text manual rename protects ownership and invalidates a captured version", () =>
    Effect.gen(function* () {
      const model = makeReadModel({ titleState: generated, titleAutoRenamedAt: NOW });
  ...
  it.effect("same-title automatic requests do not advance ownership or badge time", () =>
    Effect.gen(function* () {
      const event = yield* decideOrchestrationCommand({
  ...
  it.effect("native refinement completes without a recurring rename badge", () =>
    Effect.gen(function* () {
      ...
      expect(completion.payload).not.toHaveProperty("titleAutoRenamedAt");
      model = yield* projectEvent(model, { ...completion, sequence: 2 });
      expect(model.threads[0]?.titleAutoRenamedAt).toBeUndefined();
