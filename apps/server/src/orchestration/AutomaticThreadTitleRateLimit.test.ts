...
const insertSuccessfulRename = (input: {
  ...
  insertEvent({
    ...
    payload: {
      ...
      titleState: {
        source: "generated",
        version: `agent-thread-title:${input.eventId}`,
...
tests("AutomaticThreadTitleRateLimit", (it) => {
  it.effect("uses age and completed turns only for the first automatic rename", () =>
    Effect.gen(function* () {
  ...
  it.effect("ignores manual title changes and idempotent requests for cooldown", () =>
    Effect.gen(function* () {
  ...
  it.effect("keeps cooldown and fresh-turn requirements after recreating the limiter", () =>
    Effect.gen(function* () {
      const threadId = ThreadId.make("restart-cooldown");
      yield* insertInitiallyEligibleThread(threadId);
      yield* insertSuccessfulRename({
      ...
      yield* TestClock.setTime(Date.parse("2026-09-12T12:01:00.000Z"));
      const query = yield* AutomaticThreadTitleRenameQuery;
      const available = yield* Effect.gen(function* () {
        return yield* (yield* AutomaticThreadTitleRateLimit).isAvailable(threadId, balancedLimit);
      }).pipe(
        Effect.provide(
  ...
  it.effect("uses stream order to select the last successful rename", () =>
    Effect.gen(function* () {
