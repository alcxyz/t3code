...
function makeHarness(
  ...
  let thread = decodeThreadShell({
    ...
    ...(options.source === "legacy"
      ...
      : {
          titleState: {
            source: options.source === "user" ? "manual" : "generated",
            version: "initial-title",
  ...
  const engine = OrchestrationEngineService.of({
    ...
    dispatch: (command) =>
      Effect.gen(function* () {
        ...
        if (command.type === "thread.title.automatic.update" && command.title !== undefined) {
